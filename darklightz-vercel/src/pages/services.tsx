            ...
            {isLoading ? (
              ...
              services?.map((service, i) => (
                <BlurReveal key={service.id} delay={i * 0.1}>
                  <TiltCard className="group relative border border-white/5 bg-black/40 backdrop-blur-md p-8 md:p-14 rounded-[2px] transition-all duration-500 hover:border-white/20">
                    <AnimatedBorderSweep />
                    ...
                    <div className="relative z-10 grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16 items-start">
                      <div className="md:col-span-4 flex flex-col h-full">
                        ...
                        <div className="w-12 h-12 mb-8 rounded-full border border-white/10 flex items-center justify-center bg-white/5 backdrop-blur-sm">
                          <ServiceIcon index={i} />
                        </div>
                        <h2 className="text-3xl font-display font-bold group-hover:text-white transition-colors">{service.title}</h2>
                      </div>
